...
TASK:
- For each sentence: 
  Every sentence in the original text MUST be preserved.
...
- For each input sentence, extract 2–15 keywords DIRECTLY from the original text, such as "drew", "park", "lake sunrise". Do not invent , paraphrase, or generalize. Do not include inferred words unless they explicitly appear in the text.
...
{ 
  "sentence":[
    ...
    SYSTEM_TOOL_PROMPT = """ You are a diligent question-answering agent. You always want to gather and verify all relevant information before producing your final answer.
...
You need to answer a question with key candidates and corresponding tags and similar_sentence. Write short answer with exact words from event whenever possible.
   Rules:
   -  For yes/no or binary questions, output 'Yes', 'No', 'Likely yes', 'Likely no'.    
   ...
   -  If the question is about fixed characteristics of person, you could first call query_personal_information. If the question is about some events, you could first call query_topic_events(if relevant).
   ...
   If the information is vague or incomplete, you may further query_personal_information, query_topic_events, query_semantic_information query_event_keywords or query_event_context.  
    ...
    - "navigate": If evidence is insufficient, immediately call the tools with the proper argument. Do NOT describe the call in text or JSON. In each round, you must call as many relevant tools as possible, rather than skipping potential ones. Only avoid calls that are clearly irrelevant.""""""Then for every existing event, produce a relevance score in [0.0, 1.0] reflecting how useful it is for answering question:
    {
      "mode": "navigate",
      "relevance_scores": {
        "D1:1": 1.0,
    ...
    You need to answer a question with key candidates and corresponding tags and similar_sentence. Write short answer infered from sentences.
       Rules:
       -  For yes/no or binary questions, output 'Yes', 'No', 'Likely yes', 'Likely no'.    
